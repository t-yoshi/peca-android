...
import org.peercast.core.ui.databinding.LogviewerLineBinding
 ...
 */
class LogViewerFragment : Fragment(), PeerCastActivity.NestedScrollFragment {
    ...
    private fun doLogParse() = lifecycleScope.launch {
    ...
    private fun decorateMessage(s: String): CharSequence {
        return SpannableStringBuilder(s).also { ssb ->
            RE_URL.findAll(s).forEach { r ->
                val start = r.range.first
                val end = r.range.last + 1

...
ThreadInfo ServMgr::glaylistThread;
...
ServMgr::ServMgr()
{
...
#else
	IP_graylist = new PTSVector<addrCont>();
	IP_blacklist = new PTSVector<addrCont>();
	...
	{
		glaylistThread.func = ServMgr::graylistThreadFunc;
		glaylistThread.active = true;
		sys->startThread(&glaylistThread);
	}
}
// -----------------------------------
...
void ServMgr::loadSettings(const char *fn)
{
	...
	if (sys->idleSleepTime < 10)
		sys->idleSleepTime = 10;
...
THREAD_PROC ServMgr::graylistThreadFunc(ThreadInfo *t)
{
	BEGIN_THREAD_PROC;
	...
	while (t->active)
	{
		...
		for (size_t i=0; i<servMgr->IP_graylist->getCount(); ++i)
		{

...
class NotificationHelper(private val service: PeerCastService) {
    ...
    private fun piBump(channelId: String) = PendingIntent.getService(service, 0,
            Intent(PeerCastService.ACTION_BUMP_CHANNEL, null, service, PeerCastService::class.java).also {
    ...
    private fun piDisconnect(channelId: String) = PendingIntent.getService(service, 0,
            Intent(PeerCastService.ACTION_STOP_CHANNEL, null, service, PeerCastService::class.java).also {
                it.putExtra(PeerCastService.EX_CHANNEL_ID, channelId)
            }, PendingIntent.FLAG_UPDATE_CURRENT)

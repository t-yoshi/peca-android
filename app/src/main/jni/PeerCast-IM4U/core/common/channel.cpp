...
void Channel::broadcastTrackerUpdate(GnuID &svID, bool force)
{
	...
	if (((ctime-lastTrackerUpdate) > 30) || (force))
	{
		...
		int numListeners = stealth ? -1 : totalListeners(); //JP-MOD ���X�i�[���B���@�\.
		int numRelays = stealth ? -1 : totalRelays(); //JP-MOD �����[���B���@�\.
...
bool	ChanHit::writeVariable(Stream &out, const String &var)
{
	...
	if (var == "rhost0")
	{
		if (servMgr->enableGetName) //JP-EX s
		{
			...
			char h_name[128];
			if (ClientSocket::getHostname(h_name,sizeof(h_name),rhost[0].ip)) // BOF�΍���ۂ�
			{
				strcat(buf,"[");
				strcat(buf,h_name);
				strcat(buf,"]");

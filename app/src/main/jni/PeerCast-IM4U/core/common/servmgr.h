...
//		return maxOutgoing ? numConnected(Servent::T_OUTGOING) >= maxOutgoing : false;
	...
	bool topmostGui; // ??O??
	bool startWithGui; // ?N??????GUI
	bool preventSS; // ?X?N???[???Z?[?o?[???}?~
	bool noVersionCheck; // bcstAtom??????o?[?W?????`?F?b?N???s?????

	int versionDNS; // DNS?????èÔ??????V?o?[?W????????
